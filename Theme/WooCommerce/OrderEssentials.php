...
class OrderEssentials
{
    ...
    /**
     * DuoLift, per joist config: [FFL min, FFL max, joint sets, risers per
     * support, feet sets], all per 10 supports. Cradles ARE always 1 per 10
     * supports; feet are NOT. Feet only appear from the fifth band up, which the
     * briefing document this was first built from got wrong by printing "1 box"
     * against all 32 rows. Taken from the "Height lookups" tab of the 31 Jul 2026
     * matrix, which was generated by running the engines.
     */
    private const DUOLIFT_TABLES = [
        'pp50' => [
            [97, 142, 0, 0, 0], [143, 162, 1, 0, 0], [163, 207, 1, 1, 0], [208, 252, 1, 2, 0],
            [253, 297, 1, 2, 1], [298, 342, 1, 3, 1], [343, 387, 1, 4, 1], [388, 432, 1, 5, 1],
        ],
        'pp125' => [
            [172, 217, 0, 0, 0], [218, 237, 1, 0, 0], [238, 282, 1, 1, 0], [283, 327, 1, 2, 0],
            [328, 372, 1, 2, 1], [373, 417, 1, 3, 1], [418, 462, 1, 4, 1], [463, 507, 1, 5, 1],
        ],
        'ds51' => [
            [98, 143, 0, 0, 0], [144, 163, 1, 0, 0], [164, 208, 1, 1, 0], [209, 253, 1, 2, 0],
            [254, 298, 1, 2, 1], [299, 343, 1, 3, 1], [344, 388, 1, 4, 1], [389, 433, 1, 5, 1],
        ],
        'ds99' => [
            [146, 191, 0, 0, 0], [192, 211, 1, 0, 0], [212, 256, 1, 1, 0], [257, 301, 1, 2, 0],
            [302, 346, 1, 2, 1], [347, 391, 1, 3, 1], [392, 436, 1, 4, 1], [437, 481, 1, 5, 1],
        ],
    ];
    ...
    private static function get_matrix(): array
    {
        ...
        foreach ($rules as $rule) {
            ...
            $normalised[] = [
                ...
                'requires_product_ids' => self::normalise_int_array($rule['requires_product_ids'] ?? []),
                'excludes_product_ids' => self::normalise_int_array($rule['excludes_product_ids'] ?? []),
            ];
        }
        ...
        foreach ($sources as $source) {
            ...
            foreach ($recommendations as $recommendation) {
                ...
                $rules[] = [
                    ...
                    'requires_product_ids' => $source['requires_product_ids'] ?? [],
                    'excludes_product_ids' => $source['excludes_product_ids'] ?? [],
                ];
            }
        }
     ...
     * calculator produces no DuoLift lines at all.
     *
    ...
    private static function get_ffl_requirements(array $source_lines, float $area, string $project_type): array
    {
        ...
        if ($needs['duolift']) {
            ...
            if ($row !== null && $multipliers !== null) {
                ...
                $map = [
                    // One box per support box, in every band.
                    'cradles' => $per_ten,
                    // These three are per band, and are absent from the lower ones.
                    'joints' => $row['joints'] > 0 ? $per_ten : 0.0,
                    'feet' => $row['feet'] > 0 ? $per_ten : 0.0,
     ...
     *
     * Conditions come in a category form and a PRODUCT form. The product form
     * exists because some rates turn on a single item that shares every category
     * with its alternatives: the hexhead screw rate changes when the build uses
     * posts, and the only thing that distinguishes a posts build from a DuoLift one
     * is the post itself, which sits in the same plas-pro and subframes categories
     * as every other Plas-Pro part.
     *
    ...
    private static function rule_conditions_met(array $rule, array $source_lines): bool
    {
        ...
        $requires_ids = self::normalise_int_array($rule['requires_product_ids'] ?? []);
        $excludes_ids = self::normalise_int_array($rule['excludes_product_ids'] ?? []);

        if (empty($requires) && empty($excludes) && empty($requires_ids) && empty($excludes_ids)) {
            return true;
        }
        ...
        foreach ($source_lines as $line) {
            $basket_ids[(int) $line['source_id']] = true;
        ...
        $basket_ids = array_keys($basket_ids);
        ...
        if (!empty($requires_ids) && empty(array_intersect($requires_ids, $basket_ids))) {
            return false;
        }

        if (!empty($excludes_ids) && !empty(array_intersect($excludes_ids, $basket_ids))) {
            return false;
        }
