...
### The live theme directory is publicly served

Verified 2026-08-03: `https://www.millboard.com/en-gb/wp-content/themes/millboard/style.css`
returns its contents to anyone. **Any file that reaches the deploy tree is world-readable at
a predictable URL.**

So `.deployignore` is a security control, not just a size optimisation. Anything that is
developer-facing rather than runtime must be excluded. `docs/`, `CODEOWNERS` and `.phpcs.xml`
were added to it for exactly this reason: without the exclusion, `docs/secrets-register.md`
would have been publicly fetchable. It holds no secret values, but it maps where every
credential lives and how to break glass, which is not something to publish.

**When adding any developer-facing file to the repository root or a new top-level
directory, add it to `.deployignore` in the same change.** Check by listing the built tree
in the `build` job log before the first production deploy that includes it.
