...
<div <?= \Granola\Helpers::build_attributes($args['attributes']); ?>>
    ...
    <?php // data-map-action feeds the map_listing_click dataLayer event, so every
    // card action is measured the same way as the popup buttons. ?>
    <div class="map__listing__actions">
        <?php if (!empty($args['email'])) { ?>
            <a class="map__listing__action map__listing__action--icon" data-map-action="email" href="mailto:<?= esc_attr($args['email']); ?>" aria-label="<?= esc_attr__('Email', 'granola'); ?>" title="<?= esc_attr__('Email', 'granola'); ?>">
        ...
        <?php if (!empty($args['phone'])) { ?>
            <a class="map__listing__action map__listing__action--icon" data-map-action="phone" data-reveal-phone href="tel:<?= esc_attr(preg_replace('/[^\d+]/', '', (string) $args['phone'])); ?>" aria-label="<?= esc_attr__('Phone', 'granola'); ?>" title="<?= esc_attr__('Phone', 'granola'); ?>">
        ...
        <?php if (!empty($args['url'])) { ?>
            <a class="map__listing__link map__listing__action" data-map-action="more_info" href="<?= esc_url($args['url']); ?>">
                <?= esc_html_x('More info', 'Map listing detail link', 'granola'); ?>
            </a>
        <?php } ?>

        <?php if (!empty($args['directions_url'])) { ?>
            <a class="map__listing__action" data-map-action="directions" href="<?= esc_url($args['directions_url']); ?>" target="_blank" rel="noopener noreferrer">
