...
class SVG
{
    ...
    public static function get(string $name, array $args = []): string
    {
        ...
        $svg_path = $args['asset'] ? self::path($args['name']) : $args['name'];

        // Not a valid SVG file path.
        if (substr($svg_path, -4, 4) !== '.svg') {
            return $svg;
        }
        ...
        if (empty($args['width']) || empty($args['height'])) {
            $svg_info = self::info($doc);
        }

        // If width was specified, set it. Otherwise, try to set it from svg_info.
        if (!empty($args['width'])) {
            $doc->documentElement->setAttribute('width', $args['width']);
        } elseif (!empty($svg_info['w'])) {
            $doc->documentElement->setAttribute('width', $svg_info['w']);
        }

        // If height was specified, set it. Otherwise, try to set it from svg_info.
     ...
     *
     * @param string|\DOMDocument $svg The SVG as a string or DOMDocument object.
     * @param array $attr_names The HTML attributes to retrieve values for.
     * @return array The attribute value as an associative array, with attribute names as keys.
     */
    public static function attrs($svg, array $attr_names): array
    {
        $attrs = [];
        ...
        if (gettype($doc) === 'object' && get_class($doc) === 'DOMDocument') {
            ...
            foreach ($attr_names as $attr_name) {
                $attrs[$attr_name] = $doc->documentElement->getAttribute($attr_name);
            }
        }
    ...
    public static function info($svg): array
    {
        ...
        if (empty($attrs['width']) || empty($attrs['height'])) {
            if (!empty($attrs['viewBox'])) {
                $viewbox_attr_parts = explode(' ', $attrs['viewBox']);
            }
        }
        ...
        if (!empty($attrs['width'])) {
            $info['w'] = $attrs['width'];
        } elseif (!empty($viewbox_attr_parts[2])) {
            $info['w'] = $viewbox_attr_parts[2];
        }

        // If we can get the height directly, use that. Otherewise try and use the viewBox value
        if (!empty($attrs['height'])) {
            $info['h'] = $attrs['height'];
        } elseif (!empty($viewbox_attr_parts[3])) {
            $info['h'] = $viewbox_attr_parts[3];
        }
